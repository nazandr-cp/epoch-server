...
func NewProofGeneratorWithDependencies(graphClient SubgraphClient, logger lgr.L) *ProofGenerator {
	return &ProofGenerator{
		calculator:         NewCalculator(),
...
func (pg *ProofGenerator) GenerateTreeFromSubsidies(ctx context.Context, vaultAddress string, subsidies []subgraph.AccountSubsidy) (*TreeResult, error) {
...
func (pg *ProofGenerator) GenerateTreeFromSubsidiesAtBlock(ctx context.Context, vaultAddress string, subsidies []subgraph.AccountSubsidy, epochInfo *EpochTimestamp) (*TreeResult, error) {
	if epochInfo == nil {
...
func (pg *ProofGenerator) GenerateHistoricalTreeFromSubsidies(ctx context.Context, epochNumber string, subsidies []subgraph.AccountSubsidy) (*TreeResult, error) {
	if pg.epochBlockManager == nil {
		return nil, fmt.Errorf("epoch block manager not initialized - use NewProofGeneratorWithDependencies")
	}
...
func (pg *ProofGenerator) BuildMerkleRootFromSubsidies(ctx context.Context, vaultAddress string, subsidies []subgraph.AccountSubsidy) ([32]byte, int64, error) {
