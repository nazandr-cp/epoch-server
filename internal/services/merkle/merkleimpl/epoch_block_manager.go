...
func NewEpochBlockManager(graphClient SubgraphClient, logger lgr.L) *EpochBlockManager {
...
func NewTimestampManager(graphClient SubgraphClient, logger lgr.L) *EpochBlockManager {
...
func (ebm *EpochBlockManager) getLatestProcessedEpochForVault(ctx context.Context, vaultAddress string) (*subgraph.Epoch, error) {
	query := `
	...
	if err := ebm.graphClient.ExecuteQuery(ctx, subgraph.GraphQLRequest{
		Query:     query,
		Variables: variables,
	}, &response); err != nil {
...
func (ebm *EpochBlockManager) getEpochByNumber(ctx context.Context, epochNumber string) (*subgraph.Epoch, error) {
