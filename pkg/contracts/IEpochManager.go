...
// IEpochManagerEpochDurationUpdated represents a EpochDurationUpdated event raised by the IEpochManager contract.
type IEpochManagerEpochDurationUpdated struct {
	NewDuration *big.Int
	Raw         *types.Log // Blockchain specific contextual infos
}

const IEpochManagerEpochDurationUpdatedEventName = "EpochDurationUpdated"

// ContractEventName returns the user-defined event name.
func (IEpochManagerEpochDurationUpdated) ContractEventName() string {
	return IEpochManagerEpochDurationUpdatedEventName
}

// UnpackEpochDurationUpdatedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event EpochDurationUpdated(uint256 newDuration)
func (iEpochManager *IEpochManager) UnpackEpochDurationUpdatedEvent(log *types.Log) (*IEpochManagerEpochDurationUpdated, error) {
	event := "EpochDurationUpdated"
	if log.Topics[0] != iEpochManager.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(IEpochManagerEpochDurationUpdated)
	if len(log.Data) > 0 {
		if err := iEpochManager.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
